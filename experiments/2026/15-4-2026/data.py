...
def normalize_math_expr(expr: str) -> str:
    ...
    boxed_match = re.match(r'^\\boxed\{(.+)\}$', s)
    if boxed_match:
...
def truncate_front_pct(text: str, pct: float = 1.0) -> str:
    """Keep only the first pct fraction of characters.

    Used for the E1 truncation-sweep experiment: crossfill readers with
    progressively longer prefixes of the CoT to measure where legibility
    emerges.

    Args:
        text: the CoT text to truncate.
        pct: fraction to keep, e.g. 0.1 = first 10%.
    """
    if not text:
        return ""
    if pct >= 1.0:
        return text
    cut = max(1, int(len(text) * pct))
    return text[:cut].rstrip()


def make_truncation_transform(pct: float) -> Callable[[str], str]:
    """Create a CoT transform that keeps the first pct fraction of the text.

    Returns a callable suitable for the cot_transform parameter of build_c2_dataset.
    """
    def transform(text: str) -> str:
        return truncate_front_pct(text, pct)
    return transform
...
def _get_reasoning_text(sample) -> str:
    """Extract reasoning text from ContentReasoning blocks in the assistant message.

    Models that use internal reasoning (e.g., DeepSeek-R1, QwQ) store
    their CoT in ContentReasoning blocks within the assistant message's
    content list. The output.completion only contains the ContentText
    portion, which may be just the final answer with no reasoning.

    Returns the concatenated reasoning text, or empty string if none found.
    """
        ...
        if isinstance(msg.content, list):
            ...
            for block in msg.content:
                if hasattr(block, "type") and block.type == "reasoning":
                    text = getattr(block, "reasoning", "") or ""
                    if text.strip():
...
def _get_completion_text(sample) -> str:
    ...
    Prefers ContentReasoning blocks (the actual CoT) over output.completion
    (which may only contain the final answer text). Falls back to
    output.completion if no reasoning blocks are found, then to
    <think>...</think> extraction from output.completion.
    """
    # First try reasoning blocks — this is the canonical CoT source
    reasoning = _get_reasoning_text(sample)
    if reasoning:
        return reasoning

    # Fall back to output.completion (works for models without reasoning blocks)
